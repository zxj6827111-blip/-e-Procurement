...
interface SettlementProjectPickerProps {
  currentProject: {
    id: string;
    code: string;
    name: string;
...
const recentStorageKey = 'eprocurement:settlement-recent-projects:v1';
...
export function SettlementProjectPicker({ currentProject }: SettlementProjectPickerProps) {
  const { currentUser, navigateToPath } = useApp();
  const containerRef = useRef<HTMLDivElement | null>(null);
  const [open, setOpen] = useState(false);
  const [query, setQuery] = useState('');
  const [debouncedQuery, setDebouncedQuery] = useState('');
  const [options, setOptions] = useState<SettlementProjectOption[]>([]);
  ...
  useEffect(() => {
    ...
    // Only a newly active project should be promoted into recent history.
  ...
  useEffect(() => {
    ...
    async function loadOptions() {
      ...
      setError('');
      try {
        const result = await loadSettlementOverview(userId, { keyword: debouncedQuery, page: 1, pageSize: 20 });
        if (active) setOptions(result.projectOptions);
      } catch (loadError) {
        if (!active) return;
        setOptions([]);
        setError(loadError instanceof Error ? loadError.message : '结算项目搜索失败。');
      } finally {
  ...
  const availableOptions = useMemo(() => {
    const currentOption: SettlementProjectOption = { ...currentProject, lastUpdatedAt: null };
  ...
  const selectProject = (projectId: string) => {
    ...
    setOpen(false);
    setQuery('');
    navigateToPath(`/settlement-materials?projectId=${encodeURIComponent(projectId)}`);
  };

  return (
    <div ref={containerRef} className="relative w-full sm:w-[360px]">
      <span className="mb-1 block text-xs font-medium text-slate-500">切换结算项目</span>
      <button
        type="button"
        data-ui-check="settlement-project-picker"
      ...
      {open ? (
        <div className="absolute right-0 z-40 mt-2 w-[420px] max-w-[calc(100vw-2rem)] overflow-hidden rounded-md border border-slate-200 bg-white shadow-xl" data-ui-check="settlement-project-picker-panel">
          <div className="border-b border-slate-200 p-3">
            <div className="relative">
              <Search className="pointer-events-none absolute left-3 top-2.5 h-5 w-5 text-slate-400" />
              <input
                autoFocus
                data-ui-check="settlement-project-picker-search"
          ...
          <div className="max-h-[360px] overflow-y-auto p-2" role="listbox" aria-label="结算项目搜索结果">
            ...
            {!loading && !error ? (
              <>
                ...
                <ProjectOptionGroup label={debouncedQuery ? '搜索结果' : '结算项目'} options={regularOptions} currentProjectId={currentProject.id} onSelect={selectProject} />
                <ProjectOptionGroup label="外部交易备案" options={externalOptions} currentProjectId={currentProject.id} onSelect={selectProject} />
                {!availableOptions.length ? <div className="px-3 py-8 text-center text-sm text-slate-500">没有匹配的结算项目。</div> : null}
              </>
            ) : null}
          </div>
        </div>
...
function ProjectOptionGroup({
  ...
  label: string;
  options: SettlementProjectOption[];
  ...
  return (
    <div className="mb-2 last:mb-0">
      ...
      {options.map((option) => {
        const statusMeta = settlementStatusMeta[option.settlementStatus];
          ...
          >
            ...
            <span className="min-w-0 flex-1">
              ...
              <span className="mt-1 block truncate text-xs text-slate-500">
                {option.supplierName}{option.settlementBillNo ? ` · ${option.settlementBillNo}` : ''}
              </span>
            </span>
