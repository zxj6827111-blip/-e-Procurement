...
export function AppProvider({
  ...
  if (projectId !== undefined && projectId !== pendingProjectIdRef.current) {
  ...
  const setCurrentView = (nextView: ViewState) => {
    if (onNavigate) {
      const target = routeForGeminiView(nextView, runtimeState?.roleId ?? '', pendingProjectIdRef.current ?? currentProjectId);
