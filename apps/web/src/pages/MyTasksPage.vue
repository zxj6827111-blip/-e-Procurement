...
function createdAfter(task: R8WorkflowTaskView) {
...
async function load() {
  ...
  error.value = "";
  try {
    tasks.value = await loadWorkflowTasks(session);
  } catch (err) {
...
async function runTaskAction(task: R8WorkflowTaskView, action: "approve" | "reject" | "complete") {
  ...
  try {
    if (action === "approve" && task.approvalInstanceId) {
      ...
      const result = await completeWorkflowTask(task.id);
      auditLogId.value = result.auditLogId ?? "";
    }
    await load();
  } catch (err) {
